import { jasmine, describe, it, beforeEach, afterEach, expect, jest } from "./jasmine.js";

describe("socker worker", () => {
  let mockCreateSocket, mockRoutes, mockRouteByData, routeFn, addEventListener, socket;

  beforeEach(() => {
    socket = {};
    mockCreateSocket = jasmine.createSpy("createSocket").and.returnValue(socket);
    mockRoutes = {
  ...
  it("should create a socket", () => {
    expect(mockCreateSocket).toHaveBeenCalledOnceWith(self.location.origin, self);
  });

  it("should listen for messages", () => {
    expect(self.addEventListener).toHaveBeenCalledOnceWith("message", routeFn, false);
  });
